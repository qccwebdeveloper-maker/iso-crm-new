import React,{useState,useEffect}from 'react';import axios from 'axios';import Layout from '../../components/common/Layout';import toast from 'react-hot-toast';import{Plus,Edit,Trash2,BookOpen,ArrowLeft,GripVertical}from 'lucide-react';import ConfirmDeleteModal from '../../components/ConfirmDeleteModal';
export default function AdminStandards(){
  const[standards,setStandards]=useState([]);const[loading,setLoading]=useState(true);const[modal,setModal]=useState(null);const[saving,setSaving]=useState(false);const[form,setForm]=useState({name:'',clauses:[],active:true});const[dragIdx,setDragIdx]=useState(null);const[deletingId,setDeletingId]=useState(null);const[confirmDel,setConfirmDel]=useState(null);
  ...
  const del=async()=>{if(deletingId||!confirmDel)return;setDeletingId(confirmDel._id);try{await axios.delete(`/api/standards/${confirmDel._id}`);toast.success('Deleted');load();setConfirmDel(null);}catch{toast.error('Failed');}finally{setDeletingId(null);}};
